...
// One PR leaves behind three artifacts: its managed git worktree (worktree.ts),
...
import { workerFileFor, workerLogFor } from './paths.ts';
import { removeSessionEntry } from './sessions.ts';
...
export function parsePrKey(prKey: unknown): { repo: string; number: number } | null {
...
export async function cleanupPr(prKey: string, nameWithOwner: string | null = null): Promise<void> {
  ...
  // (c) Prune the prKey entry from sessions.json. removeSessionEntry (sessions.ts) runs the
