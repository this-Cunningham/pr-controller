...
import { DEFAULT_SENSITIVITY, clampSensitivity } from './sensitivity.ts';
...
export const clampPoll = (n: unknown | number): number => {
...
// Every field is an optional flat override of a Config field, plus the profile-selection keys.
// Parsed from untrusted JSON, so all fields are optional and narrowed by the `||`/`??` reads below.
type Profile = {
  host?: string;
  owner?: string;
  login?: string;
...
type LocalConfig = Partial<Config> & {
  profile?: string;
...
if (hasLocalConfig) {
  // config.local.json is external data — JSON.parse is `any`; cast to the partial-override shape.
  try { local = JSON.parse(readFileSync(localPath, 'utf8')) as LocalConfig; }
  catch (e) { console.error(`[config] config.local.json failed to parse — ignoring it (${e instanceof Error ? e.message : String(e)})`); }
}
...
export const config: Config = {
  ...
  // The `'PRC_ONLY_PRS' in env` guard guarantees a string on that branch (TS doesn't narrow
  // process.env from `in`); the else-branch always resolves to an array since the built-in
  // profiles set onlyPRs: [] — cast preserves that without changing the `||` runtime.
  onlyPRs: ('PRC_ONLY_PRS' in env ? csv(env.PRC_ONLY_PRS as string) : (local.onlyPRs || base.onlyPRs)) as string[],
