...
import { isBehindBase, needsRebase, needsJira, deriveDisposition, applyDebugReviewer } from './rules.ts';
import type { Pr, Thread, ThreadWithDisposition, WorkerResult, Action, BranchHealth } from './types.ts';

export function deriveRecord(
  pr: Pr,
  { workerResult = null, outOfSync = false, agentError = null }: {
  ...
  const actions = new Map<string, Action>((workerResult?.actions || []).map((a) => [a.threadId, a]));
  pr.threads = (pr.threads || []).map((raw: Thread): ThreadWithDisposition => {
    ...
    // deriveDisposition lives in the still-untyped rules.ts; its verdict is one of the
    // Disposition literals — narrow the loosely-inferred return to the canonical shape.
    const verdict = deriveDisposition(t, a) as { disposition: ThreadWithDisposition['disposition']; reason: string };
    return { ...t, ...verdict, suggestedReply: a?.suggestedReply, suggestedApproach: a?.suggestedApproach };
  });
