...
export const workerFileFor = (repo: string, number: number) => join(DATA, `worker-${repo}-${number}.json`);
export const workerLogFor = (repo: string, number: number) => join(DATA, `worker-${repo}-${number}.log`);
