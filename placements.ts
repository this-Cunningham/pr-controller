...
// item of a PR belongs to. Pure, no I/O, locked by test/placements.test.ts.
//
...
import type { Disposition, Lane, Placement, Pr, SubjectKind, ThreadWithDisposition } from './types.ts';
...
export const LANES: Lane[] = ['needs', 'progress', 'waiting'];
...
export const LANE_OF_DISPOSITION: Record<Disposition, Lane | null> = {
...
export function laneOf(disposition: string): Lane | null {
  return Object.prototype.hasOwnProperty.call(LANE_OF_DISPOSITION, disposition)
    ? LANE_OF_DISPOSITION[disposition as Disposition]
    : 'needs';
}
...
export function placementsFor(pr: Pr): Placement[] {
  const prKey = `${pr.repo}#${pr.number}`;
  const rows: Placement[] = [];
  const push = (subjectKind: SubjectKind, subjectId: string, disposition: Disposition, reason?: string) => {
  ...
  // By the time placementsFor runs, derive.deriveRecord has attached the worker's
  // verdict, so each thread is a ThreadWithDisposition.
  ((pr.threads || []) as ThreadWithDisposition[]).forEach((t, i) => {
...
export function prSortRank(rows: Placement[]): number {
