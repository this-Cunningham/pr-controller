...
// The app-authored files are now .ts/.tsx; lint them with the TypeScript parser so the
// same DS-adherence rules (prop/tone contracts, token hygiene, the local controller-prop
// rules) keep covering the converted SwimlaneBoard.tsx / main.tsx. The frozen design-system
// and prototype-synced feature components stay .jsx and are covered by the js/jsx block.
const jsConfig = {
  files: ["src/**/*.{js,jsx}"],
  languageOptions: {
    ecmaVersion: "latest",
...
const tsConfig = {
  ...jsConfig,
  files: ["src/**/*.{ts,tsx}"],
  languageOptions: { ...jsConfig.languageOptions, parser: tsParser },
};
