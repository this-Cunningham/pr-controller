...
// locked with tests (see test/rules.test.ts). server.ts and scanner.ts import
...
const includesAny = (name: string | null | undefined, list: string[]): boolean =>
...
export function applyDebugReviewer(thread: Thread, login = config.login, debugToken = config.debugToken): Thread {
...
export function dispatchable(thread: Thread, login = config.login, token = config.triggerToken): boolean {
...
export function validateWorkerResult(raw: unknown): { result: WorkerResult | null; problems: string[] } {
  const problems: string[] = [];
  if (!raw || typeof raw !== 'object' || Array.isArray(raw))
    return { result: null, problems: ['result is not a JSON object'] };
  // Untrusted model-written JSON: narrowed field-by-field below.
  const rawObj = raw as Record<string, unknown>;
  if (rawObj.actions !== undefined && !Array.isArray(rawObj.actions))
    return { result: null, problems: ['`actions` is present but not an array'] };

  const actions: Action[] = [];
  for (const [i, a] of ((rawObj.actions as unknown[]) || []).entries()) {
    if (!a || typeof a !== 'object') { problems.push(`action[${i}] is not an object`); continue; }
    const action = a as Record<string, unknown>;
    if (!action.threadId) { problems.push(`action[${i}] missing threadId`); continue; }
    if (!WORKER_RESPONSES.includes(action.response as string)) {
      problems.push(`action[${i}] (${action.threadId}) has invalid response ${JSON.stringify(action.response)}`);
      continue;
    }
    actions.push(a as Action);
  }

  const bh = rawObj.branchHealth as { surfaced?: unknown; ciReran?: unknown } | undefined;
  ...
  return { result: { ...rawObj, actions } as WorkerResult, problems };
}
...
export function isWorkerResultStale(
  result: WorkerResult | null | undefined,
  liveThreadIds: Set<string> | Iterable<string> | null | undefined,
  { needsRebase = false, outOfSync = false }: { needsRebase?: boolean; outOfSync?: boolean } = {},
): boolean {
...
export function isBranchHealthResultStale(
  result: WorkerResult | null | undefined,
  { needsRebase = false, checkState = null }: { needsRebase?: boolean; checkState?: CheckState | null } = {},
  liveThreadIds?: Set<string> | Iterable<string> | null,
): boolean {
...
export function mergePending(
  pendingMap: Map<string, Thread>,
  incoming: Iterable<Thread> | null | undefined,
): Map<string, Thread> {
...
export function categorizeChecks(
  failing: Check[] | null | undefined,
  cfg: Pick<Config, 'ignoreChecks' | 'complianceChecks'> = config,
): { codeChecks: Check[]; complianceChecks: Check[] } {
...
export function needsJira(
  title: string | null | undefined,
  complianceChecks: Check[] | null | undefined,
  pattern = config.jiraPattern,
): boolean {
...
export function isBehindBase(
  mergeState: MergeStateStatus | null | undefined,
...
export function dispatchDecision({ newThreadCount = 0, ciFailing = false, needsRebase = false, healthChanged = false, rebaseSurfaced = false, rebaseErrored = false, ciReran = false }: {
  ...
  rebaseSurfaced?: boolean;
  rebaseErrored?: boolean;
  ciReran?: boolean;
} = {}): { kind: 'rebase' | 'feedback' | 'none' } {
  if (needsRebase)
...
export function nextSeenThreads(
  prevSeen: Iterable<string>,
  liveFps: Iterable<string>,
  needsRebase: boolean,
): Set<string> {
...
export function inScope(prKey: string, onlyPRs = config.onlyPRs): boolean {
...
export function cloneUrl(
  ...
  { host = config.host, protocol = config.gitProtocol }: { host?: string; protocol?: Config['gitProtocol'] } = {},
): string {
...
// The stream-json terminal `result` event. Model/CLI-written, so fields are optional and
// narrowed where read (classifyRunOutcome). Not a cross-module record — local to the worker layer.
...
export function parseTerminalResult(out: string = ''): TerminalEvent | null {
  const lines = String(out || '').split('\n');
  for (let i = lines.length - 1; i >= 0; i -= 1) {
    const line = lines[i].trim();
    if (!line || line[0] !== '{') continue;
    try {
      // External data: stream-json line from the worker's captured stdout/stderr.
      const ev = JSON.parse(line) as TerminalEvent;
...
export function shouldRetryWorker(failures: number = 0, max = config.workerMaxRetries): boolean {
  return failures < max;
}
