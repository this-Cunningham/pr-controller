...
import type { Pr, Thread, Check, BranchHealth, Placement, StateJson, PollError } from './types.ts';
import { config, ghEnv, hasLocalConfig, cloneRootDefaulted, clampPoll } from './config.ts';
import { DATA, STATE, DECISIONS, workerFileFor } from './paths.ts';

import { scanAll, scanOnePr } from './scanner.ts';
import { spawnDiscussTerminal, runWorker, readWorkerResult, drainWorkers, wasInterrupted } from './worker.ts';
import { ensureWorktree } from './worktree.ts';
import { cleanupPr } from './cleanup.ts';
import { dispatchable, dispatchDecision, nextSeenThreads, isWorkerResultStale, isBranchHealthResultStale } from './rules.ts';
import { deriveRecord } from './derive.ts';
import { placementsFor, prSortRank, LANES } from './placements.ts';
...
// A caught value is `unknown` under strict mode; these handlers only ever read .message/
// .stack off it (Error-shaped at runtime). This alias narrows at the read site without
// changing the runtime guards (`e.message || e`, `e.stack || e.message`).
type ErrLike = { message?: string; stack?: string };

// Prepend "[TICKET]" to the PR title to satisfy the compliance check.
async function setPrJira(pr: Pr, ticket: string) {
...
async function postThreadReply(threadId: string, body: string) {
...
// Seeded as a partial idle snapshot; writeState() reassigns the full StateJson shape on
// every poll/refresh and the daemon seeds full state at startup before any request lands.
let state: StateJson = { updatedAt: null, scope: config.onlyPRs || [], prs: [], pollingEnabled: false } as unknown as StateJson;
// prKey -> { Set of "threadId:lastCommentId" seen last poll, branch-health signature } for diff detection.
const seen = new Map<string, { threads: Set<string>; health: string }>();
...
const outOfSyncPRs = new Set<string>();
...
const agentErrorPRs = new Map<string, string>();
...
let lastPollError: PollError | null = null;
...
const fp = (t: Thread) => `${t.threadId}:${t.lastCommentId}`;
...
async function deriveAndSetPrFields(pr: Pr) {
  ...
  const liveThreadIds = new Set((pr.threads ?? []).filter((t: Thread) => !t.error && t.threadId).map((t: Thread) => t.threadId));
...
async function writeState(prs: Pr[]) {
  ...
  prs.sort((a, b) => (a.sortRank ?? 0) - (b.sortRank ?? 0));
  state = {
...
async function refreshOnePR(prKey: string) {
  // scanner.scanOnePr resolves a PR-shaped record (or null when gone); pin it to Pr at
  // this module boundary until scanner.ts exports a precise Promise<Pr | null>.
  const pr = (await scanOnePr(prKey)) as Pr | null;
  if (!pr) {  // no longer open / in scope (merged/closed) — drop it + reclaim its leftovers
    ...
    try { await cleanupPr(prKey, old?.nameWithOwner); } catch (e) { log.error(`refreshOnePR cleanup ${prKey} failed`, (e as ErrLike).message); }
...
dispatcher.init({
  events, ensureWorktree,
  // worker.runWorker types `opts` as fully-optional (RunWorkerOpts) and resolves a
  // wider result than the dispatcher's Deps shape; the variance is benign (every
  // per-run field is set at the call site), so pin it to the dependency's contract.
  runWorker: runWorker as Parameters<typeof dispatcher.init>[0]['runWorker'],
  refreshOnePR, outPath: outPathFor,
...
async function poll() {
  ...
  try {
    ...
    for (const pr of prs) {
      ...
      const prKey = `${pr.repo}#${pr.number}`;
      const h: Partial<BranchHealth> = pr.branchHealth || {};
      const prev = seen.get(prKey) || { threads: new Set(), health: '' };
      const newThreads = (pr.threads ?? []).filter((t: Thread) => !t.error && !prev.threads.has(fp(t)) && dispatchable(t));
      const healthSig = `${h.mergeable}|${h.mergeState}|${h.checkState}|${(h.failingChecks||[]).map((c: Check) => c.name + c.state).join(',')}`;
      ...
      const liveFps = (pr.threads ?? []).filter((t: Thread) => !t.error).map(fp);
    ...
    for (const [k, oldPr] of prevByKey) {
      ...
      try { await cleanupPr(k, oldPr?.nameWithOwner); } catch (e) { log.error(`cleanup ${k} failed`, (e as ErrLike).message); }
    ...
    lastPollError = { at: new Date().toISOString(), message: String((e as ErrLike).message || e) };
    log.error('poll failed', (e as ErrLike).stack || (e as ErrLike).message);
    try { await writeState(state.prs); } catch (e2) { log.error('persist after poll failure failed', (e2 as ErrLike).message); }
  } finally {
    polling = false;
  }
}
...
async function startPolling() {
  if (pollTimer) return;
  pollingEnabled = true;
  try { await writeState(state.prs); } catch (e) { log.error('persist on start failed', (e as ErrLike).message); }
  events.notifyStateUpdated();
  poll();
...
async function stopPolling() {
  ...
  try { await writeState(state.prs); } catch (e) { log.error('persist on stop failed', (e as ErrLike).message); }
  events.notifyStateUpdated();
}
...
// `edits` is the untrusted POST /config body (JSON.parse → unknown); each access is
// guarded at runtime (Array.isArray / typeof / != null), so model it as a loose record.
async function applyConfigEdits(edits: Record<string, unknown>) {
  ...
  if (edits.workerSensitivity != null) {
    config.workerSensitivity = clampSensitivity(edits.workerSensitivity as number | string); changed.push('workerSensitivity');
  }
...
async function persistConfigLocal(flat: Record<string, unknown>) {
  const path = join(config.baseDir, 'config.local.json');
  let existing: Record<string, unknown> = {};
  // config.local.json is operator-written JSON; parse as unknown then treat as a flat record.
  try { existing = JSON.parse(await readFile(path, 'utf8')) as Record<string, unknown>; } catch {}
  await writeFile(path, JSON.stringify({ ...existing, ...flat }, null, 2) + '\n');
}

async function recordDecision(payload: Record<string, unknown>) {
  ...
  // data/decisions.json is our own append log; parse as unknown then treat as a record array.
  try { all = JSON.parse(await readFile(DECISIONS, 'utf8')) as Record<string, unknown>[]; } catch {}
  all.push({ ...payload, at: new Date().toISOString() });
  await writeFile(DECISIONS, JSON.stringify(all, null, 2));
}

const server = createServer(async (req: IncomingMessage, res: ServerResponse) => {
  // req.url is always set for an HTTP request (typed string | undefined on IncomingMessage).
  const url = new URL(req.url as string, `http://localhost:${config.port}`);
  if (req.method === 'GET' && url.pathname === '/') {
    ...
    try {
      ...
      httpLog.error('serve index failed', (e as ErrLike).message);
  ...
  if (req.method === 'GET' && hasDist && url.pathname.startsWith('/assets/')) {
    ...
    if (existsSync(file)) {
      ...
      try {
        ...
        httpLog.error('serve asset failed', (e as ErrLike).message);
        try { res.writeHead(500, { 'content-type': 'text/plain' }); res.end('error'); } catch {}
      }
      return;
    }
  ...
  if (req.method === 'POST' && url.pathname === '/polling') {
    ...
    req.on('end', async () => {
      try {
        ...
        httpLog.error('/polling failed', (e as ErrLike).message);
  ...
  if (req.method === 'POST' && url.pathname === '/config') {
    ...
    req.on('end', async () => {
      try {
        ...
        httpLog.error('/config failed', (e as ErrLike).message);
        res.writeHead(500, { 'content-type': 'application/json' });
        res.end(JSON.stringify({ ok: false, error: String((e as ErrLike).message || e) }));
      }
    });
  ...
  if (req.method === 'POST' && url.pathname === '/decision') {
    ...
    req.on('end', async () => {
      try {
      // POST /decision body is untrusted (model/UI-written); parse as unknown then read
      // its string/array fields with runtime guards.
      const payload = JSON.parse(body || '{}') as Record<string, unknown>;
      await recordDecision(payload);
      // `spawn` is reassigned across the action branches; widen it to cover every outcome
      // shape (spawnDiscussTerminal's { spawned, resumed }, plus the action/queued results).
      let spawn: { spawned: boolean; reason?: string; action?: string; queued?: boolean; resumed?: boolean } =
        { spawned: false, reason: `unknown action: ${payload.action}` };
      if (payload.action === 'discuss') {
        ...
        if (!pr || (payload.threadId && !thread)) spawn = { spawned: false, reason: 'PR or thread not found' };
          ...
          // `thread` is Thread | undefined here; spawnDiscussTerminal's param is Thread | null
          // (it only truthiness-checks it), so the cast is safe and passes the value unchanged.
          spawn = await spawnDiscussTerminal(pr, thread as Thread | null, wt.path, thread ? null : (payload.kind as string | null));
        }
      }
      if (payload.action === 'note') {
        const pr = state.prs.find((p) => `${p.repo}#${p.number}` === payload.prKey);
        const thread = pr?.threads!.find((t) => t.threadId === payload.threadId);
        const body = ((payload.note as string) || '').trim();
      ...
      if (payload.action === 'set-jira') {
        const pr = state.prs.find((p) => `${p.repo}#${p.number}` === payload.prKey);
        const ticket = ((payload.ticket as string) || '').trim();
        ...
        if (!pr) spawn = { spawned: false, reason: 'PR not found' };
          ...
          await refreshOnePR(payload.prKey as string);
      ...
      if (payload.action === 'run-agent') {
        const pr = state.prs.find((p) => `${p.repo}#${p.number}` === payload.prKey);
        const threadIds = (Array.isArray(payload.threadIds) ? payload.threadIds : []) as string[];
        if (!pr) spawn = { spawned: false, reason: 'PR not found' };
        else if (!threadIds.length) spawn = { spawned: false, reason: 'no approved threads' };
        else {
          const queued = dispatcher.isWorking(payload.prKey as string);
      ...
      res.end(JSON.stringify({ ok: true, scope: config.onlyPRs || [], spawn }));
        ...
        httpLog.error('/decision failed', (e as ErrLike).message);
        try { res.writeHead(500, { 'content-type': 'application/json' });
          res.end(JSON.stringify({ ok: false, error: String((e as ErrLike).message || e) })); } catch {}
      }
    });
    return;
  }
...
catch (e) { srvLog.warn('could not resolve gh account (gh api user) — is gh authed for this host?', String((e as ErrLike).message || e).slice(0, 160)); }
if (account && !config.login) config.login = account;

// Graceful shutdown. A process kill (SIGTERM from a redeploy/launchd, SIGINT from
// Ctrl-C) must NOT orphan in-flight `claude` workers — see drainWorkers (worker.ts)
...
server.listen(config.port, async () => {
  ...
  try { await writeState([]); } catch (e) { srvLog.error('initial writeState failed', (e as ErrLike).message); }
