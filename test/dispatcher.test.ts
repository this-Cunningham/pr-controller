...
import type { Pr, Thread } from '../types.ts';

// The dispatcher's injected-deps shape, pulled straight off its init() signature so the
// mocks below are checked against the real contract. The tests also inject an extra
// `notifyStateUpdated` event hook (the daemon's real events surface has it; the dispatcher
// never calls it), so widen the events shape locally to allow it without weakening the module.
type DispatcherDeps = Parameters<typeof dispatcher.init>[0];
type TestDeps = Omit<DispatcherDeps, 'events'> & {
  events: DispatcherDeps['events'] & { notifyStateUpdated?: () => void };
};

// One captured run as the harness records it (only the fields the assertions read).
interface RunRecord { prKey: string; threadCount: number; rebase: boolean; applyApproved: boolean; }

function harness(): RunRecord[] {
...
test('enqueueRebase with no threads still dispatches a rebase-only worker run', async () => {
  const runs = harness();
  const pr = { repo: 'site-vdp-remix', number: 835, branchHealth: {} } as unknown as Pr;
...
test('enqueue with no threads but ci:true still dispatches a CI-fix worker run', async () => {
  const runs = harness();
  const pr = { repo: 'inv', number: 11, branchHealth: { checkState: 'FAILING', failingChecks: [{ name: 'e2e-sandbox CI', state: 'FAILURE' }] } } as unknown as Pr;
...
test('enqueue with no threads and no ci/rebase reason does NOT dispatch', async () => {
  const runs = harness();
  const pr = { repo: 'noop', number: 1, branchHealth: {} } as unknown as Pr;
...
test('a thrown worker run re-stages its batch and retries on the next enqueue', async () => {
  ...
  const pr = { repo: 'rstage', number: 1, threads: [{ threadId: 't1' }], branchHealth: {} } as unknown as Pr;
  dispatcher.enqueue(pr, [{ threadId: 't1' }] as unknown as Thread[], {});
  await flush();
  ...
  dispatcher.enqueue(pr, [{ threadId: 't2' }] as unknown as Thread[], {});
  await flush();
  assert.equal(runs.length, 1);
...
test('a spawned-but-failed run surfaces an error and re-stages its batch', async () => {
  ...
  const pr = { repo: 'softfail', number: 1, threads: [{ threadId: 't1' }], branchHealth: {} } as unknown as Pr;
  dispatcher.enqueue(pr, [{ threadId: 't1' }] as unknown as Thread[], {});
  await flush();
...
function failHarness() {
  ...
  const ctl: { outcome: { ok: boolean; reason: string | null } } = { outcome: { ok: false, reason: 'boom' } };
  dispatcher.init({
    events: { markStarted() {}, markFinished() {}, notifyStateUpdated() {} },
    ensureWorktree: async () => ({ path: '/tmp/wt', outOfSync: false }),
...
test('a worker that keeps failing trips the breaker and stops auto-retrying after the cap', async () => {
  ...
  const pr = { repo: 'cap', number: 1, threads: [{ threadId: 't1' }], branchHealth: {} } as unknown as Pr;
  ...
  assert.match(String(errors.at(-1)), /kept failing after \d+ attempts/);  // terminal surface persists
...
test('a clean run resets the failure counter (restores the full retry budget)', async () => {
  ...
  const pr = { repo: 'reset', number: 1, branchHealth: {} } as unknown as Pr;
...
test('a genuinely new signal (fresh feedback or manual Re-run) resets a tripped breaker', async () => {
  ...
  const pr = { repo: 'newsig', number: 1, threads: [{ threadId: 't1' }], branchHealth: {} } as unknown as Pr;
  for (let i = 0; i < config.workerMaxRetries + 2; i += 1) { dispatcher.enqueue(pr, [{ threadId: 't1' }] as unknown as Thread[], {}); await flush(); }
  ...
  dispatcher.enqueue(pr, [{ threadId: 't2' }] as unknown as Thread[], {}); await flush();
  ...
  dispatcher.enqueue(pr, [{ threadId: 't1' }, { threadId: 't2' }] as unknown as Thread[], { reset: true }); await flush();
...
test('a repeatedly-erroring rebase retries then parks at the cap (bounded by the breaker)', async () => {
  ...
  const pr = { repo: 'reb', number: 1, threads: [], branchHealth: {} } as unknown as Pr;
  ...
  assert.match(String(errors.at(-1)), /kept failing after \d+ attempts/);  // parked as terminal workerFailed
...
test('forget() during a running worker keeps the lock (no double-dispatch)', async () => {
  ...
  const pr = { repo: 'tomb', number: 1, threads: [], branchHealth: {} } as unknown as Pr;
  dispatcher.enqueueRebase(pr, {});
  await flush();
  assert.equal(runs.length, 1);
...
test('an interrupted prior run triggers worktree recovery + a recovered resume', async () => {
  const seen: { recover?: boolean; recovered?: boolean } = {};
  dispatcher.init({
    events: { markStarted() {}, markFinished() {}, notifyStateUpdated() {} },
    ensureWorktree: async (pr, o: { recover?: boolean } = {}) => { seen.recover = o.recover; return { path: '/tmp/wt', outOfSync: false, recovered: !!o.recover }; },
  ...
  const pr = { repo: 'rec', number: 1, threads: [], branchHealth: {} } as unknown as Pr;
  dispatcher.enqueueRebase(pr, {});
  await flush();
...
test('markFinished carries pending=true while a queued batch remains, false when drained', async () => {
  ...
  const pr = { repo: 'pend', number: 1, threads: [{ threadId: 'a' }, { threadId: 'b' }], branchHealth: {} } as unknown as Pr;
...
test('enqueueApproved honors rebaseOnConflict (an approved run on a conflicted PR rebases)', async () => {
  const runs = harness();
  const pr = { repo: 'conf', number: 7, threads: [{ threadId: 'a' }], branchHealth: {} } as unknown as Pr;
...
test('a failed CI-only run restores its ci reason and retries on the next enqueue', async () => {
  ...
  const pr = { repo: 'cifail', number: 1, threads: [], branchHealth: {} } as unknown as Pr;
