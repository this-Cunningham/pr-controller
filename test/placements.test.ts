...
import { placementsFor, prSortRank, laneOf, LANE_OF_DISPOSITION, DISPOSITION_RANK } from '../placements.ts';
import type { Disposition, Placement, Pr } from '../types.ts';

// A thread override as the test data-builder accepts it — a loose subset of
// ThreadWithDisposition (`id` maps to threadId; disposition/error may be absent
// for the scan-failure case, which placementsFor handles via t.error first).
...
// The overrides the pr() data-builder accepts — a subset of the derived Pr fields
// placementsFor reads, plus loose threads.
interface PrOverrides {
  ...
  liveStatus?: Pr['liveStatus'] | 'idle';
  threads?: ThreadOverride[];
}

// Build a derived-PR-shape record. `threads` take { id, disposition, reason, error }.
function pr(over: PrOverrides = {}): Pr {
  // Test fixture: an intentionally partial derived-PR record carrying only the
  // fields placementsFor reads. Cast to Pr (placementsFor's input contract).
  return {
...
const subjects = (rows: Placement[]) => rows.map((r) => r.subjectId);
