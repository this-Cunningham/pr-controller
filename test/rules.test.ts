...
import type { Thread, Action, WorkerResult, Check, Config } from '../types.ts';
...
test('dispatchable: reviewer had the last word -> dispatch', () => {
  assert.equal(dispatchable({ lastAuthor: 'jheipler', lastBody: 'please fix' } as Thread, ME, TOKEN), true);
});

test('dispatchable: my plain annotation -> no dispatch', () => {
  assert.equal(dispatchable({ lastAuthor: ME, lastBody: 'note for future reviewers' } as Thread, ME, TOKEN), false);
});

test('dispatchable: my reply (waiting on reviewer) -> no dispatch', () => {
  assert.equal(dispatchable({ lastAuthor: ME, lastBody: 'done, ptal' } as Thread, ME, TOKEN), false);
});

test('dispatchable: my comment WITH trigger token -> dispatch', () => {
  assert.equal(dispatchable({ lastAuthor: ME, lastBody: 'this is gross @claude-plz-fix' } as Thread, ME, TOKEN), true);
});

test('dispatchable: reviewer replies on MY thread -> dispatch (keys on last author)', () => {
  assert.equal(dispatchable({ lastAuthor: 'jheipler', lastBody: 'actually, change this' } as Thread, ME, TOKEN), true);
});

test('dispatchable: after bot replies "fixed" as me -> no re-dispatch', () => {
  assert.equal(dispatchable({ lastAuthor: ME, lastBody: 'fixed' } as Thread, ME, TOKEN), false);
});
...
test('dispatchable: my comment WITH @claude-debug -> dispatch', () => {
  assert.equal(dispatchable({ lastAuthor: config.login, lastBody: 'seed this @claude-debug' } as Thread), true);
});

const ME2 = 'ccunningham';

test('deriveDisposition: worker surfaced -> needsYourApproval with the code-cited reason', () => {
  const r = deriveDisposition({ lastAuthor: 'jheipler' } as Thread, { response: 'surface', reason: 'breaks the guard' } as Action, ME2);
...
test('deriveDisposition: worker fixed -> agentAutoFixed (distinct from praise)', () => {
  const r = deriveDisposition({ lastAuthor: ME2 } as Thread, { response: 'fix', reason: 'fixed it' } as Action, ME2);
  assert.equal(r.disposition, 'agentAutoFixed');
});

test('deriveDisposition: worker praised -> agentAcknowledged (distinct from fix)', () => {
  assert.equal(deriveDisposition({ lastAuthor: ME2 } as Thread, { response: 'praise' } as Action, ME2).disposition, 'agentAcknowledged');
});

test('deriveDisposition: no worker action, reviewer last word -> notYetReviewed', () => {
  assert.equal(deriveDisposition({ lastAuthor: 'jheipler' } as Thread, undefined, ME2).disposition, 'notYetReviewed');
});

test('deriveDisposition: no worker action, I replied last -> awaitingReviewer', () => {
  assert.equal(deriveDisposition({ lastAuthor: ME2 } as Thread, undefined, ME2).disposition, 'awaitingReviewer');
});

test('deriveDisposition: thread error -> agentError', () => {
  assert.equal(deriveDisposition({ error: 'scan failed' } as Thread, undefined, ME2).disposition, 'agentError');
});
...
test('validateWorkerResult: drops malformed actions, keeps the good ones', () => {
  ...
  const { result, problems } = validateWorkerResult(raw);
  assert.deepEqual(result!.actions!.map((a: Action) => a.threadId), ['a']);
...
test('mergePending: dedupes by threadId, skips errored / id-less threads', () => {
  ...
  mergePending(m, [{ threadId: 'a', body: 'v1' }, { threadId: 'b' }] as Thread[]);
  mergePending(m, [{ threadId: 'a', body: 'v2' }, { error: 'scan failed' }, { body: 'no id' }] as Thread[]);
...
test('isWorkerResultStale: no actions match a live thread + clean branch -> stale', () => {
  const result = { actions: [{ threadId: 'gone', response: 'fix' }] } as WorkerResult;
...
test('isBranchHealthResultStale: never drops a live thread verdict, and no-op without a flag', () => {
  const withLiveThread = { actions: [{ threadId: 'a', response: 'fix' }], branchHealth: { ciReran: true } } as WorkerResult;
...
test('needsJira: compliance failing + no ticket in title -> true', () => {
  assert.equal(needsJira('Migrate agent-router to single-agent', [{ name: 'compliance/sox' }] as Check[], cfg.jiraPattern), true);
});

test('needsJira: compliance failing + ticket present -> false', () => {
  assert.equal(needsJira('[CNAI-277] chip copy', [{ name: 'compliance/sox' }] as Check[], cfg.jiraPattern), false);
});
...
test('applyDebugReviewer: my comment with the token -> re-attributed to a reviewer', () => {
  const t = applyDebugReviewer(
    { author: 'someone', lastAuthor: ME, lastBody: `surface this @claude-debug` } as Thread, ME, DBG);
...
test('applyDebugReviewer: also rewrites author when I opened the thread too', () => {
  const t = applyDebugReviewer(
    { author: ME, lastAuthor: ME, lastBody: `@claude-debug` } as Thread, ME, DBG);
  assert.equal(t.author, DEBUG_REVIEWER);
  assert.equal(t.lastAuthor, DEBUG_REVIEWER);
});

test('applyDebugReviewer: my comment WITHOUT the token -> untouched', () => {
  const t = applyDebugReviewer({ author: ME, lastAuthor: ME, lastBody: 'plain note' } as Thread, ME, DBG);
  assert.equal(t.lastAuthor, ME);
});

test('applyDebugReviewer: a real reviewer comment -> untouched', () => {
  const t = applyDebugReviewer({ author: 'jheipler', lastAuthor: 'jheipler', lastBody: 'fix @claude-debug' } as Thread, ME, DBG);
  assert.equal(t.lastAuthor, 'jheipler');
});

test('applyDebugReviewer: disabled (no token configured) -> untouched', () => {
  const t = applyDebugReviewer({ author: ME, lastAuthor: ME, lastBody: '@claude-debug' } as Thread, ME, '');
  assert.equal(t.lastAuthor, ME);
});

test('applyDebugReviewer: errored thread -> untouched', () => {
  const t = applyDebugReviewer({ error: 'scan failed' } as Thread, ME, DBG);
...
test('inScope: empty/null allowlist -> all PRs in scope', () => {
  ...
  assert.equal(inScope('site-vdp-remix#835', null as unknown as string[]), true);
...
test('cloneUrl: ssh (default) vs https, configurable host — no hardcoded transport', () => {
  ...
  assert.equal(
    cloneUrl('o/r', { host: 'h', protocol: 'whatever' as Config['gitProtocol'] }),
    'git@h:o/r.git');
