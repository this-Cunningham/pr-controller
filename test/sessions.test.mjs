...
import { loadSessions, updateSessions, removeSessionEntry } from '../sessions.mjs';
...
test('updateSessions writes the mutated map; a mutate returning false skips the write', async () => {
  ...
  await updateSessions((m) => { m['a#1'] = { id: 'x' }; }, p);
...
test('concurrent updateSessions calls serialize — neither update is lost', async () => {
  ...
  await Promise.all([
    updateSessions((m) => { m['a#1'] = { id: 'x' }; }, p),
    updateSessions((m) => { m['b#2'] = { id: 'y' }; }, p),
  ]);
