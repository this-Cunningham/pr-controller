...
import { drainWorkers, readWorkerResult } from '../worker.mjs';
...
function capturingLog() {
  ...
  const rec = (lvl) => (msg, extra) => calls[lvl].push(extra !== undefined ? `${msg} ${extra}` : String(msg));
...
async function withResultFile(text, fn) {
